...
# This script converts media files to AAC/M4A, relying dffmpeg.sh-script for libfdk_aac codec support
# Supports setting encoder quality, deleting original files, and raising ffmpeg log level
...
Options:
  -q,     Set the quality of the encoding (0-5). Default is 5.
...
while getopts ":q:ldh" opt; do
  case ${opt} in
    q)
      if [ -n "${OPTARG}" ] && [ "${OPTARG}" -ge 0 ] && [ "${OPTARG}" -le 5 ]; then
        QUALITY=${OPTARG}
      else
        echo "Error: Argument for -q is missing or out of range. QUALITY should be between 0 and 5." >&2
        exit 1
      fi
    ;;
    l) LOGLEVEL=${OPTARG} ;;
...
convertFile() {
  ...
  if [ -f "$FILE" ]; then
    ...
    if [ $SKIP -eq 0 ]; then
      # Convert file
      dffmpeg.sh -i "$BASENAME" -f "-vn -sn -dn -nostats -loglevel $LOGLEVEL -c:a libfdk_aac -afterburner 1 -vbr $QUALITY" "$NEW_FILE"
      ...
      if [ $? -eq 0 ]; then
        ...
        NEW_SIZE=$(du -b "$NEW_FILE_PATH" | cut -f1)
